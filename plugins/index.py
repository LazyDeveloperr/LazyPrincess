...
async def send_for_index(bot, message):
    ...
    if message.from_user.id in ADMINS:
        if (LAZY_MODE==True):
            ...
            buttons = [
                ...
                [ InlineKeyboardButton('ꜱᴛᴀʀᴛ ɪɴᴅᴇx',callback_data=f'index#accept#{chat_id}#{last_msg_id}#{message.from_user.id}')],
    ...
    if (LAZY_MODE == True):
        if message.from_user.id in LAZY_RENAMERS:
            ...
            await message.reply('🎉\n\n\n❤️ Thank You For the Contribution, Wait For My Moderators to verify the files.\n\n\n🎁')
            buttons = [
                        [InlineKeyboardButton("ʀᴇɴᴀᴍᴇ", callback_data="requireauth") ],
                        [InlineKeyboardButton('ᴄʟᴏꜱᴇ', callback_data='cancel')]]
            reply_markup = InlineKeyboardMarkup(buttons)
            file = getattr(message, message.media.value)
            filename = file.file_name
